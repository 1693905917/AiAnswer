...
import com.hzp.aixiaoda.annotation.AuthCheck;
import com.hzp.aixiaoda.config.VipSchedulerConfig;
import com.hzp.aixiaoda.exception.ThrowUtils;
import com.hzp.aixiaoda.manager.AiManager;
import com.hzp.aixiaoda.model.dto.question.*;
import com.hzp.aixiaoda.model.entity.App;
import com.hzp.aixiaoda.model.enums.AppTypeEnum;
import com.hzp.aixiaoda.service.AppService;
import com.hzp.aixiaoda.service.UserService;
import com.hzp.aixiaoda.common.BaseResponse;
import com.hzp.aixiaoda.common.DeleteRequest;
import com.hzp.aixiaoda.common.ErrorCode;
import com.hzp.aixiaoda.common.ResultUtils;
import com.hzp.aixiaoda.constant.UserConstant;
...
import com.hzp.aixiaoda.model.vo.QuestionVO;
import com.hzp.aixiaoda.service.QuestionService;
...
public class QuestionController {
     ...
     * AI生成问题列表
     *
     * @param aiGenerateQuestionRequest AI生成问题请求对象
     * @return BaseResponse<List<QuestionContentDTO>> AI生成问题列表的响应对象
     */
    ...
    @GetMapping("/ai_generate/sse") //一定要用Get请求
    public SseEmitter aiGenerateQuestionSSE(AiGenerateQuestionRequest aiGenerateQuestionRequest,HttpServletRequest request) {
        ...
        // 左括号计数器，除了默认值外，当回归为 0 时，表示左括号数等于右括号数，可以截取
        AtomicInteger counter = new AtomicInteger(0); //这个就是属于力扣的一个算法题：有效的括号
        // 拼接完整题目
        ...
        //TODO 属于扩展性业务  可以给用户创建VIP用户专用线程池 这个可以用admin
        if("vip".equals(loginUser.getUserRole())){
            scheduler = vipScheduler;
        }
        modelDataFlowable
                .observeOn(scheduler)//被观察者采用IO流
                .map(modelData -> modelData.getChoices().get(0).getDelta().getContent())//获取智谱AI发送的数据
                .map(message -> message.replaceAll("\\s", ""))//去掉智谱AI发送的数据中的空格
                .filter(StrUtil::isNotBlank)
                .flatMap(message -> {//除去空格智慧，把每一个非空格的字符串拆分成一个一个的字符
                    List<Character> characterList = new ArrayList<>();
                    for (char c : message.toCharArray()) {
                        characterList.add(c);//把每一个非空格的字符串拆分成一个一个的字符，放入List中
                    }
                    return Flowable.fromIterable(characterList);
                })
                .doOnNext(c -> {//通过判断{}来截取获得每一个题目
                    // 如果是 '{'，计数器 + 1
                    if (c == '{') {
                        counter.addAndGet(1);
                    }
    ...
    //仅测试隔离线程池使用
    @Deprecated
    @GetMapping("/ai_generate/sse/test") //一定要用Get请求
